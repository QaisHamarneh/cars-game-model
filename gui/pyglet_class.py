...
class CarsWindow(pyglet.window.Window):
    def __init__(self, game, agents, eval_games, train_games=0, manual=False):
        super().__init__()
    ...
    def on_draw(self):
        self.clear()
        if self.n_games < self.train_games + self.eval_games:
            ...
            for shape in self.car_shapes:
                shape.draw()
        elif self.n_games < self.train_games + self.eval_games:
            self.close()
            quit()
        self.frames_count += FRAME_RATE if self.n_games < self.train_games else FRAME_RATE // 6

    def _update_game(self):
        for player in range(self.game.players):
            if not self.gameover[player]:
                self.old_positions[player] = self.game.cars[player].pos
                if self.n_games_trained < self.train_games:
                    self.gameover[player] = self.agents[player].train()
                else:
    ...
    def _update_cars(self):
        ...
        for player, car in enumerate(self.game.cars):
            ...
            #     a=car.size, b=BLOCK_SIZE,
            #     color=car.color if not car.dead else DEAD_GREY)
            # car_shape.rotation = 90 if horiz_direction[car.direction] else 0
            # self.car_shapes.append(car_shape)
            # # , x
