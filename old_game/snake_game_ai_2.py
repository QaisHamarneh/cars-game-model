...
class SnakeGameAI2:
    ...
    def reset(self):
        ...
        self.heads = [Point(self.w/2, i * self.h/(self.players + 1))
                      for i in range(1, self.players + 1)]

        self.snakes = [[self.heads[i],
                        Point(self.heads[i].x + (-1)**i * self.block_size,
                              self.heads[i].y),
                        Point(self.heads[i].x + (-1)**i * (2*self.block_size), self.heads[i].y)]
    ...
    def _place_food(self, player):
        ...
        y = random.randint(0, (self.h-self.block_size) //
                           self.block_size)*self.block_size
        while any([Point(x, y) in self.snakes[i] for i in range(self.players)]):
            x = random.randint(0, (self.w-self.block_size) //
                               self.block_size)*self.block_size
            y = random.randint(0, (self.h-self.block_size) //
                               self.block_size)*self.block_size
    ...
    def play_step(self, player, action):
        ...
        # 2. move
        old_dist_to_food = dist(self.heads[player], self.foods[player])
        self._move(player, action)  # update the head
        self.snakes[player].insert(0, self.heads[player])

        new_dist_to_food = dist(self.heads[player], self.foods[player])
        ...
        if self.heads[player] == self.foods[player]:
    ...
    def is_collision(self, player, pt=None):
        if pt is None:
            pt = self.heads[player]
        # hits boundary
        if pt.x > self.w - self.block_size or pt.x < 0 or pt.y > self.h - self.block_size or pt.y < 0:
            return True
        # hits itself
        if pt in self.snakes[player][1:]:
            return True
        if any([pt in self.snakes[i] for i in range(self.players) if i != player]):
            return True
        return False
    ...
    def _move(self, player, action):
        ...
        x = self.heads[player].x
        y = self.heads[player].y
        ...
        self.heads[player] = Point(x, y)

    def _update_ui(self):
        self.display.fill(BLACK)

        for player in range(self.players):
            for pt in self.snakes[player]:
